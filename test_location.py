...
def test_location(tc_directory: PathLike | str = '..',
    ...
    selected_stations = [stations[station['ril100']] for station in station_json.data]
    random_indices = [random.randint(0, len(selected_stations)) for _ in range(10)]
    random_stations = [selected_stations[i] for i in random_indices]
    for station in random_stations:
        if station.location:
            real_location = station.location
            auto_located = with_location_data(station)
            auto_location = auto_located.location
            delta = real_location.distance(auto_location)
            print(delta)
...
if __name__ == '__main__':
    ...
    test_location(args.tc_directory, args.data_directory)
